...
bool inBet(ll a, ll b, ll c)    {
    if(c<a) swap(a,c); return a <= b && b <= c; }
...
void solve()
{
    ...
    ll mn=INT_MAX,ans=0;
    ll val=b[n]; b.pop_back();
    for(ll i = 0; i < n; i++)
    {
        ans += abs(a[i]-b[i]);
        mn=min(abs(a[i]-val),mn);
        mn=min(abs(val-b[i]),mn);
        if (min(a[i], b[i]) <= val && val <= max(a[i], b[i]))
        mn = 0;
    }
    cout << ans+mn+1 << endl;
}
