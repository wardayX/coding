...
void solve() {
    ...
    ll sp = (t.first-s.first)*(t.first-s.first) + (t.second-s.second)*(t.second-s.second);
    for(ll i = 0; i < n; i++) {
        ll d1 = ((t.first-a[i].first) * (t.first-a[i].first)) + 
                ((t.second-a[i].second) * (t.second-a[i].second));
        if(sp >= d1) {
            chk = 1;
            break;
        }
